...
export const update = async (req, res) => {
    try {
        const { id } = req.params; // Obtén el ID del evento a actualizar
        const { hotel, description, date, time, additional } = req.body; // Obtén los nuevos datos del cuerpo de la solicitud
        ...
        // Actualiza los campos proporcionados, excluyendo hotel y additional
        Object.assign(eventoExistente, updateData);

        // Guarda el evento actualizado en la base de datos
        await eventoExistente.save();

        // Devuelve el evento actualizado
        return res.send({ message: 'Event updated successfully', data: eventoExistente });
    } catch (error) {
        console.error(error);
...
export const find = async (req, res) => {
    try {
        let data = await Event.find().populate({
            path: 'additional',
